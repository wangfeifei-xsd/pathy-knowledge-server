...
- **上传**：`POST /api/v1/media/upload`（`multipart/form-data`：`file` + 可选 `title`）；支持图片、视频、**APK**；单文件默认上限 **200MB**（`MEDIA_MAX_UPLOAD_BYTES`），总配额见 `MEDIA_TOTAL_QUOTA_BYTES`。经网关（如 Tengine）转发时须同步调大 `client_max_body_size`（示例见 `deploy/nginx-client-max-body-size.example.conf`）。  
