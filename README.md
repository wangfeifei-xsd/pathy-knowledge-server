...
6. **合并去重 + rerank + 截断**（`_merge_and_rerank_candidates` + `_trim_context`）  
   双路候选合并去重后做融合重排，取前 `top_k_chunks`，再按 `context_budget_chars` 块级截断。注入格式为 `### 文件相对路径` + 可选 `**标题路径**` + 正文（正文内 wiki 多媒体占位符会剔除，且**不**在注入块中附带媒体 code）；`merged_media` 返回本次命中片段中解析出的、已在 manifest 登记的媒体摘要（按命中顺序合并、按 code 去重）。

7. **输出**  
   - `recall_method` 字段为 `hybrid_bm25_vector`。  
   - `query_terms` 为过滤停用词后实际参与打分的词项。  
   - `recall_hits[].score` 为融合重排得分（越大越相关）。  
   - `recall_hits[].heading_path` 为该片段的 Markdown 标题路径（`父 > 子`），无标题切块时为空字符串。  
   - `merged_media` 为各命中片段正文内解析出的多媒体，按命中顺序合并、再按 `code` 去重后的列表（便于一次展示或拉取二进制）；`recall_hits` 条目不包含逐条 `media` 字段。

### 多媒体层（`media/`）

与 `raw/wiki/schema` 并列：`DATA_ROOT/media/`（启动时创建），`media/manifest.json` 登记条目，二进制在 `media/objects/…`。

- **上传**：`POST /api/v1/media/upload`（`multipart/form-data`：`file` + 可选 `title`、`target_folder`）；`target_folder` 为 `media/` 下子目录，空则默认 `objects/aa/bb/…`；支持图片、视频、**APK**；单文件默认上限 **200MB**（`MEDIA_MAX_UPLOAD_BYTES`），总配额见 `MEDIA_TOTAL_QUOTA_BYTES`。经网关（如 Tengine）转发时须同步调大 `client_max_body_size`（示例见 `deploy/nginx-client-max-body-size.example.conf`）。  
- **列表**：`GET /api/v1/media/items`（manifest 条目，供控制台；路由在 `/{code}` 之前避免 `code=items` 歧义）。  
- **下载**：`GET /api/v1/media/{code}`（支持 Range，便于视频播放）。  
- **反向索引**：`POST /api/v1/media/reindex-backrefs` 扫描 wiki 写入 `.pathy/media_backrefs.json`；`GET /api/v1/media/{code}/backrefs`。  
- **用量**：`GET /api/v1/media/meta/summary`。
